...
<div class="row">
<div class="col-md-6">@include('web.partials.field', ['field' => 'client_id', 'label' => 'Client', 'inputType' => 'select', 'options' => ['' => 'Select client'] + $clients->pluck('name', 'id')->all()])</div>
<div class="col-md-3">@include('web.partials.field', ['field' => $type.'_date', 'label' => ucfirst($type).' date', 'inputType' => 'date', 'value' => $record->{$type.'_date'} ?? now()->format('Y-m-d')])</div>
...
@if($type !== 'job') @include('web.partials.field', ['field' => 'terms', 'label' => 'Terms', 'inputType' => 'textarea']) @endif
@include('web.partials.attachments', ['editing' => true])
<button type="submit" class="btn btn-primary">Save {{ ucfirst($type) }}</button>
@if(auth()->user()->hasPermission('view_'.$resource))<a class="btn btn-outline-secondary" href="{{ route($resource.'.index') }}">Back to list</a>@endif
</form>
